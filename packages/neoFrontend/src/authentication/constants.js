...
export const GITHUB_OAUTH_URL = `${OAUTH_URL_BASE}?client_id=${GITHUB_CLIENT_ID}&state=${stateEncoder(
  'github'
)}`;

export const GOOGLE_OAUTH_URL = `https://accounts.google.com/o/oauth2/v2/auth?scope=openid email https://www.googleapis.com/auth/userinfo.profile&client_id=${GOOGLE_CLIENT_ID}&redirect_uri=${REDIRECT_URI}&response_type=id_token&nonce=${Math.random()}&state=${stateEncoder(
  'google'
)}`;
export const FACEBOOK_OAUTH_URL = `https://www.facebook.com/v8.0/dialog/oauth?client_id=${FACEBOOK_CLIENT_ID}&response_type=code&redirect_uri=${REDIRECT_URI}&state=${stateEncoder(
  'fb'
)}`;
