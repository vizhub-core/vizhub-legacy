...
export const useVizInfos = (vizInitialInfosToTrack = []) => {
  ...
  const vizInfosIds = useMemo(() => vizInitialInfos.map(({id}) => id), [vizInitialInfos]);
  ...
  const vizInfos$ = useMemo(() => {
    ...
    return Object.assign({}, hookLifetimeVizInfosByIdRef.current, addedVizInfosById);
  }, [vizInitialInfos]);
  ...
  const getPrevious = useCallback((id) => vizInfos$[id].getValue(), [vizInfos$]);
